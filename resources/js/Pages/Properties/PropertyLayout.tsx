...
const PropertyLayout = ({ children }: PropsWithChildren) => {
    ...
    const t = usePage().url.split("/");
    const endPoint = t[t.length - 1];

    console.log();
    return (
        <Authenticated>
            <div className="grid gap-4 grid-cols-[auto,1fr] h-full">
                <section className="card flex flex-col max-h-full gap-6">
                    <div
                        className={
                            "flex gap-3 " +
                            (endPoint == "" ? "font-bold text-blue-900" : "")
                        }
                    >
                        <svg
